...
当 NVDA 界面语言为中文时，Polyglot 会使用离线英汉词典扩展查看光标所在的单词命令：

1.  按一次 `numpad5`（台式机键盘布局） 读出查看光标所在的单词。
2. 连按两次逐字朗读该单词。
3. 连按三次，如果本地词典命中完整单词，则朗读中文释义。

调用 NVDA `speech.spellTextInfo` 的插件也会获得相同行为。
如果整行只有一个英文单词，连按三次读出当前行命令也可获得释义。
查询支持一层常见的首末句子标点、安全的拼写变体，以及常见的复数、时态、分词、比较级和最高级形式。精确词条始终优先；如果一个拼写可能对应多个释义，Polyglot 会朗读最多三个候选词条及其首义。两至三个字符的全大写单词如果只命中小写词条，会朗读可能的小写释义，并提示它也可能是缩写。外形合法但未收录的英文单词会明确提示未收录。单字符、多词行和不支持的文本仍保留 NVDA 原有的字符解释。
